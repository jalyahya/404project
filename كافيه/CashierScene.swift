...
class CashierScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        closeButton.size = CGSize(width: 50, height: 70)
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let node = draggedHand,
              let name = node.name,
              draggableItemNames.contains(name),
              !InventoryManager.shared.hasUsed(name) else {
            draggedHand = nil
            originalPosition = nil
            return
        }
        ...
        let returnAction = SKAction.group([moveBack, scaleBack])
        node.run(returnAction)
